...
import com.tencent.tdsql.mysql.cj.util.DnsSrv;
import com.tencent.tdsql.mysql.cj.util.DnsSrv.SrvRecord;
    ...
    protected void fixProtocolDependencies(Map<String, String> hostProps) {
        ...
        if (!isNullOrEmpty(protocol) && protocol.equalsIgnoreCase("PIPE")) {
            if (!hostProps.containsKey(PropertyKey.socketFactory.getKeyName())) {
                hostProps.put(PropertyKey.socketFactory.getKeyName(), "com.tencent.tdsql.mysql.cj.protocol.NamedPipeSocketFactory");
            }
        }
    }
