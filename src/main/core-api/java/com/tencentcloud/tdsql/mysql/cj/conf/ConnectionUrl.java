...
import com.tencentcloud.tdsql.mysql.cj.util.DnsSrv;
import com.tencentcloud.tdsql.mysql.cj.util.DnsSrv.SrvRecord;
...
public abstract class ConnectionUrl implements DatabaseUrlContainer {
    ...
    public enum Type {
        ...
        FAILOVER_DNS_SRV_CONNECTION("jdbc:tdsql-mysql+srv:", HostsCardinality.ONE_OR_MORE, "com.tencentcloud.tdsql.mysql.cj.conf.url.FailoverDnsSrvConnectionUrl"), //
        LOADBALANCE_DNS_SRV_CONNECTION("jdbc:tdsql-mysql+srv:loadbalance:", HostsCardinality.ONE_OR_MORE, "com.tencentcloud.tdsql.mysql.cj.conf.url.LoadBalanceDnsSrvConnectionUrl"), //
        REPLICATION_DNS_SRV_CONNECTION("jdbc:tdsql-mysql+srv:replication:", HostsCardinality.ONE_OR_MORE, "com.tencentcloud.tdsql.mysql.cj.conf.url.ReplicationDnsSrvConnectionUrl"), //
        XDEVAPI_DNS_SRV_SESSION("tdsql-mysqlx+srv:", HostsCardinality.ONE_OR_MORE, "com.tencentcloud.tdsql.mysql.cj.conf.url.XDevApiDnsSrvConnectionUrl"), //
        // Standard schemes:
        SINGLE_CONNECTION("jdbc:tdsql-mysql:", HostsCardinality.SINGLE, "com.tencentcloud.tdsql.mysql.cj.conf.url.SingleConnectionUrl", PropertyKey.dnsSrv, FAILOVER_DNS_SRV_CONNECTION), //
        FAILOVER_CONNECTION("jdbc:tdsql-mysql:", HostsCardinality.MULTIPLE, "com.tencentcloud.tdsql.mysql.cj.conf.url.FailoverConnectionUrl", PropertyKey.dnsSrv,
                FAILOVER_DNS_SRV_CONNECTION), //
        LOADBALANCE_CONNECTION("jdbc:tdsql-mysql:loadbalance:", HostsCardinality.ONE_OR_MORE, "com.tencentcloud.tdsql.mysql.cj.conf.url.LoadBalanceConnectionUrl", PropertyKey.dnsSrv,
        ...
        public static Type fromValue(String scheme, int n) {
            for (Type t : values()) {
                if (t.getScheme().equalsIgnoreCase(scheme) && (n < 0 || t.getCardinality().assertSize(n))) {
                    return t;
                } else if (t.getScheme().replaceAll("tdsql-", "").equalsIgnoreCase(scheme)
                        && (n < 0 || t.getCardinality().assertSize(n))) {
                    return t;
                }
            }
            if (n < 0) {
    ...
    protected void fixProtocolDependencies(Map<String, String> hostProps) {
        ...
        if (!isNullOrEmpty(protocol) && protocol.equalsIgnoreCase("PIPE")) {
            if (!hostProps.containsKey(PropertyKey.socketFactory.getKeyName())) {
                hostProps.put(PropertyKey.socketFactory.getKeyName(), "com.tencentcloud.tdsql.mysql.cj.protocol.NamedPipeSocketFactory");
            }
        }
    }
